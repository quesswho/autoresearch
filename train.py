...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        param_groups = [
            ...
            dict(kind='adamw', params=value_embeds_params, lr=embedding_lr * dmodel_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=VE_WEIGHT_DECAY),
...
WINDOW_PATTERN = "L"    # all full-causal: routes every layer through SDPA flash (no banded mask)
...
WEIGHT_DECAY = 0.2      # Muon WD lowered 0.3->0.2: underfit regime wants less reg on transformer matrices (VE_WD untouched)
VE_WEIGHT_DECAY = 0.1   # decoupled AdamW weight decay on value embeddings (per-token memorization table; was 0.0)
...
MTP_ENABLE = True
MTP_K = 2                # predict t+1 (head 0) .. t+K; K=2 (one t+2 head) is OPTIMAL (K=3 gave sub-thresh val_bpb -0.0021 but no benchmark gain -> reverted)
...
SWA_START = 0.6  # begin averaging once progress >= this fraction of training (sweep: 0.5=1.3312, 0.7=1.3273, 0.8=1.3383; pin the optimum near 0.7)
